...
class Employee:
    ...
    def __init__(self, root):
        ...
        B_Update = Button(Frame1in, text="UPDATE", width=10, command=self.update_data)
        ...
        B_Del = Button(Frame1in, text="DELETE", width=10, command=self.delete_data)
        ...
        B_Clear = Button(Frame1in, text="CLEAR", width=10, command=self.clear)
        ...
        B_Search = Button(Frame2, text="SEARCH", width=10, command=self.search_data)
        B_Search.grid(row=0, column=3, padx=10,pady=15)
        
        Pmw.Balloon(Frame1in).bind(B_Search, 'Dugme za pretragu podataka\nNakon izbora kljucne reci klikni za pretragu.')  # binding it and assigning a text to it
        
        B_Show = Button(Frame2, text="SHOW ALL", width=10, command=self.displayData)
        B_Show.grid(row=0, column=4, padx=10,pady=15)
        
        Pmw.Balloon(Frame1in).bind(B_Show, 'Dugme za pregled svih podataka\nKlikni za pregled svih podataka.')  # binding it and assigning a text to it
        ...
        self.Emp.bind("<ButtonRelease-1>",
                      self.get_data)  # OVDE POVEZUJEMO KLIK NA RED U TREEVIEW SA POZIVANJEM FUNKCIJE get_data

        self.Emp.pack(fill=BOTH, expand=1)

    def displayData(self):
        # con = mysql.connector.connect(host='localhost', user='root', database='off')
    ...
    def addData(self):
        ...
        cur = con.cursor()
        if self.Eno.get() == "":
            messagebox.showinfo("Oprez", "Niste popunili Eno")
        elif self.Name.get() == "":
            messagebox.showinfo("Oprez", "Niste popunili Name")
        elif self.Department.get() == "":
            messagebox.showinfo("Oprez", "Niste popunili Department")
        elif self.Gender.get() == "":
            messagebox.showinfo("Oprez", "Niste popunili Gender")
        elif self.Address.get("1.0", END).strip() == "":
            messagebox.showinfo("Oprez", "Niste popunili Address")
        else:
            print(self.Eno.get())
            print(self.Name.get())
            ...
            sql = "INSERT INTO employee (EMPNO, NAME, DEPARTMENT, GENDER, ADDRESS) VALUES (?,?,?,?,?)"
            args = (
    ...
    def get_data(self, ev):  # OVA FUNKCIJA PUNI KOMPONENTE U FREJMU frame1, POTREBNO JE NAVESTI DRUGI ARGUMENT
        ...
        print(
            contents)  # {'text': '', 'image': '', 'values': [3, 'Marko', 'CS', 'Male', 'BG\n'], 'open': 0, 'tags': ''}
    ...
    def update_data(self):
        ...
        if self.Eno.get() == "":
            ...
            cur.execute("UPDATE employee SET name=?, department=?,gender=?,address=? where empno=?",
                        (self.Name.get(), self.Department.get(), self.Gender.get(), self.Address.get("1.0", END),
                         self.Eno.get()))
            con.commit()
        con.close()
        self.displayData()
    ...
    def delete_data(self):
        ...
        if self.Eno.get() == "":
            ...
            cur.execute("delete from employee where empno=?", self.Eno.get())
            con.commit()
        con.close()
        self.displayData()
        self.clear()
    ...
    def search_data(self):  # POVEZATI SA KOMPONENTOM C_Search (textvariable=self.Search),
        ...
        con = sqlite3.connect('test.db')
        cur = con.cursor()
        print("select * from employee where " + str(
            self.SearchBy.get()) + " like '%" + self.SearchText.get() + "%'")
        cur.execute("select * from employee where " + str(
            self.SearchBy.get()) + " like '%" + self.SearchText.get() + "%'")
        rows = cur.fetchall()
        if len(rows) != 0:
            self.Emp.delete(*self.Emp.get_children())  # OVDE PRVO BRIŠEMO POSTOJEĆE ZAPISE
            ...
            self.Emp.delete(*self.Emp.get_children())  # U SLUCAJU POGRESNE PRETRAGE, BRISANJE REDOVA
        con.close()
