...
class Employee:
    def __init__(self, root):
        ...
        B_Clear = Button(Frame1in, text="CLEAR", width=10, command=self.clear_data)
    ...
    def displayData(self):
        con = mysql.connector.connect(host='localhost', user='root', database='off', password='', port=3307)
        cur = con.cursor()
    ...
    def search_data(self):
        con = mysql.connector.connect(host='localhost', user='root', database='off', port=3307)
        cur = con.cursor()
        cur.execute("select * from employee where " + str(
            self.SearchBy.get()) + " like binary '%" + self.SearchText.get() + "%'")
        rows = cur.fetchall()
        if (len(rows) != 0):
            self.Emp.delete(*self.Emp.get_children())  # OVDE PRVO BRIŠEMO POSTOJEĆE ZAPISE
            for row in rows:  # OVDE PUNIMO TREEVIEW
    ...
    def delete_data(self):
        con = mysql.connector.connect(host='localhost', user='root', database='off', password='', port=3307)
        cur = con.cursor()
        sql_delete = "DELETE FROM employee WHERE empno = %s"
        sql_data = (self.Eno.get(),)
        cur.execute(sql_delete, sql_data)
        con.commit()
        con.close()
        self.displayData()
        self.clear_data()
